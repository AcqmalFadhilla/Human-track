...
class Detect(object):
    ...
    def detect(self, img: np.ndarray, box_type="xywh") -> Tuple[np.ndarray]:
        """Deteksi objek.
        Pengembalian:
           Tuple[np.ndarray]: Bentuk setiap elemen adalah (25500, 4) (25500,) (25500,).
        """
    ...
    def _detect(self, img: np.ndarray):
        ...
        output_data = self.interpreter.get_tensor(self.output_details[0]["index"])  # mengambil tensor  x(1, 25200, 7)
        return output_data
    ...
    def postprocess(self, output_data, box_type: str) -> Tuple[np.ndarray]:
        ...
        # Filter berdasarkan ambang batas keyakinan.
    ...
    def to_xyxy(self, boxes: np.ndarray) -> np.ndarray:
        """Merubah xywh ke xyxy."""
        # (x, y) adalah koordinat titik tengah kotak.
